...
class product:
    ...
    def __init__(self,root):
        ...
        #create object reference instance od databse class as p
        p=Database()
        p.conn()
        ...
        #function to close operations the product frame
        def close():
        ...
        #add to scroll bar
        def productRec(event): # function to be called scrollbar productlist
        ...
        #update / search the record from database table
        def search():
        ...
        self.ITitle = Label(HeadFrame, font=('arial',60,'bold'),
                            fg='red',text='Warehouse Inventory Sales Purchase ',bg='white')
        self.ITitle.grid()

        OperationFrame =Frame(MainFrame,bd=2,width=1524,height=60,
                               padx=50,pady=20,bg='white',relief=RIDGE)
        OperationFrame.pack(side=BOTTOM)
        
        BodyFrame =Frame(MainFrame,bd=2,width=1524,height=500,
...
class Database:
    ...
    def update(self,pid="",name="",price="",qty="",company="",contact=""):
        ...
        cur.execute("update product set pid=? or pname=? or price=? or \
                  qty=? or company=? or contact=? where pid=?",(pid,name,price,qty,company,
                                                                contact,pid))
        con.commit()
        con.close()
